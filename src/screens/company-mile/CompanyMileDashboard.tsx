      ...
      <LinearGradient colors={[C.gradStart, C.gradEnd]} style={s.header}>
        <SafeAreaView>
          <View style={s.headerTop}>
            <Text style={s.headerTitle}>Per Mile Driver</Text>
            ...
            {loads.map((load) => (
              <View key={load.id} style={s.loadCard}>
                ...
                <Text style={s.loadDetail}>
                  {load.paidMileage} mi × ${load.centsPerMile?.toFixed(2)}/mi = <Text style={{ fontWeight: '700', color: C.text }}>{fmt((load.paidMileage ?? 0) * (load.centsPerMile ?? 0))}</Text>
                </Text>
