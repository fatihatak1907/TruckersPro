  ...
  async function handleDeleteWeek(weekKey: string) {
    Alert.alert('Delete Week', 'Delete all loads for this week?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive',
        onPress: async () => {
          await deleteWeekData('company-commission', weekKey);
                    ...
                    {weekData[wk].loads.map((load) => (
                      <View key={load.id} style={s.loadCard}>
                        ...
                        <Text style={s.loadDetail}>
                          {fmt(load.earnings ?? 0)} × {((load.commissionRate ?? 0) * 100).toFixed(0)}% = <Text style={s.bold}>{fmt((load.earnings ?? 0) * (load.commissionRate ?? 0))}</Text>
                        </Text>
