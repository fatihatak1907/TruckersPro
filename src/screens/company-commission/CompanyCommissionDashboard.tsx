...
export function CompanyCommissionDashboard({ navigation }: Props) {
  ...
  const { paid, togglePaid } = usePeriodPaid('company-commission');
            ...
            {loads.map((load) => (
              <View key={load.id} style={s.loadCard}>
                ...
                <Text style={s.loadDetail}>
                  {fmt(load.earnings ?? 0)} × {((load.commissionRate ?? 0) * 100).toFixed(0)}% = <Text style={s.loadDetailBold}>{fmt((load.earnings ?? 0) * (load.commissionRate ?? 0))}</Text>
                </Text>
