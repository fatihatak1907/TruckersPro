...
export function OwnerOpHistory({ navigation, route }: Props) {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      getPaidPeriodKeys(driverType).then(setPaidKeys);
    }, [])
  );
  ...
  async function handleDeleteWeek(weekKey: string) {
    showDialog('Delete Week', 'Delete all data for this week?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive',
        onPress: async () => {
  ...
  return (
    <View style={s.root}>
      ...
      <ScreenHeader
        title="History"
        subtitle={driverType === 'lease' ? 'Lease Driver — all weeks' : 'Owner Operator — all weeks'}
      />
            ...
            {expanded === wk && weekData[wk] && (
              <View style={s.weekContent}>
                ...
                <View style={s.netRow}>
                  ...
                  <Text style={[s.netValue, { color: weekData[wk].summary.netProfit >= 0 ? C.success : C.danger }]}>
...
const s = StyleSheet.create({
  ...
  weekCard: {
    ...
    shadowColor: '#000', shadowOpacity: 0.3, shadowRadius: 10, shadowOffset: { width: 0, height: 3 }, elevation: 2,
    overflow: 'hidden',
  },
